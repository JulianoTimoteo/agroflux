...
export async function renderUsuarios() {
  ...
  tbody.innerHTML = page.length ? page.map(u => {
    ...
    return `<tr class="${ativo?'':'row-meta-no'}"><td class="td-l">${u.Nome||''}</td><td><code>${u.Email||''}</code></td><td>${dataCriacao}</td><td><span class="badge bdg-${norm(u.Nivel||'operador')}">${u.Nivel||''}</span>${resInfo}${fieldInfo}</td><td><span class="badge ${ativo?'bdg-ativo':'bdg-inativo'}">${ativo?'Ativo':'Inativo'}</span></td><td>${isSelf?'<span style="font-size:.6rem;color:var(--muted)">🔒 você</span>':`${editBtn} ${btnAction}`}</td></tr>`;
...
export async function salvarUsuario() {
  ...
  if (!editUid) {
    ...
    try {
      ...
      await signOut(authB);
      const profile = { Nome: nome, Email: email, Login: login, Nivel: nivel, admin: (nivel === 'master' || nivel === 'admin'), Abas: abas, Equipes: equipes, Ativo: false, criadoEm: serverTimestamp() };
    ...
    try {
      const updates = { Nome: nome, Email: email, Login: login, Nivel: nivel, admin: (nivel === 'master' || nivel === 'admin'), Abas: abas, Equipes: equipes };
